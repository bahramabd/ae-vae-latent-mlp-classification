...
class VariationalAutoencoder(nn.Module):
    def __init__(self, input_dim=8100, latent_dim=50):
...
def train_vae(model, dataloader, epochs, lr, beta=1.0):
    ...
    for epoch in range(epochs):
        model.train()
        total_epoch_loss, recon_epoch_loss, kl_epoch_loss = 0, 0, 0

        for x, _, _ in dataloader:
            x = x.to(device)
            optimizer.zero_grad()
            recon, mu, logvar = model(x)
            loss, recon_loss, kl_div = vae_loss(recon, x, mu, logvar, beta)
            loss.backward()
            optimizer.step()
        ...
        print(
            f"Epoch {epoch + 1}/{epochs} | Total: {total_losses[-1]:.4f} | Recon: {recon_losses[-1]:.4f} | KL: {kl_losses[-1]:.4f}")
...
def main():
    data_path = "/kaggle/input/adnidataset/"
    ...
    model = train_vae(model, dataloader, epochs, lr, beta)
